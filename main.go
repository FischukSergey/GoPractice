...
// stepsDay перебирает все записи слайс, подсчитывает и возвращает
...
func stepsDay(storage []string) int {
	...
	for _, str := range storage {
		_, step, _ := parsePackage(str)
		// if ok {  //проверка на корректность обработки строки
		 totalStep += step
		// }
	}
	return totalStep
...
func calories(distance float64) float64 {
	kilocal := (K1*Weight + (Speed*Speed/Height)*K2*Weight) * distance / Speed / 60
	return kilocal
}
...
Вы сожгли %.2f ккал.
%s`, t.Format("15:04:05"), allSteps, distance/1000, energy, achiev)
	showMessage(msg)
	// 11. Вернуть storage
